...
func getRoute(graph [][]int, start, goal int) []int {
	ret := []int{1}
	visited := make([]bool, len(graph))
	visited[1] = true
	getRouteHelper(graph, goal, 1, &ret, visited)
	return ret
}

func getRouteHelper(graph [][]int, goal, curr int, retRoute *[]int, visited []bool) bool {
	if curr == goal {
		return true
	}
	...
	for _, v := range graph[curr] {
		if !visited[v] {
			visited[v] = true
			*retRoute = append(*retRoute, v)
			if getRouteHelper(graph, goal, v, retRoute, visited) {
				return true
			}
		}
	}

	*retRoute = (*retRoute)[:len(*retRoute)-1]
	return false
}
